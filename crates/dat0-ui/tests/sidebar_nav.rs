...
/// for the event-bus drain, so each test says when a command lands
/// (`window_routing.rs` runs the real drain); the chord table, the registry
/// and the router are all production code.
#[component]

...
fn menu_local(id: &str, events: &AppEvents, ws: crate::state::Workspace) {
    ...
    // most of these: each opens a picker, a dialog or a workspace in this
    // window. The window's own items (dat0's off macOS) and the two links run.
    let the_windows = crate::menu::WINDOW_ITEMS.contains(&id);
    let a_link = matches!(id, menu_ids::DOCS | menu_ids::GITHUB);
    if ws.modal.peek().is_some() && !the_windows && !a_link {
    ...
    match id {
        ...
        | menu_ids::MINIMIZE
        | menu_ids::ZOOM
        | menu_ids::FULLSCREEN => crate::launch::manage_window(id),

...
/// # Status
///
/// **`#[ignore]`** — the T9 placeholder `assets/demo.dat0` (one 10-row/3-col
/// `demo_data` table, no chart, no pipeline, no saved query) cannot satisfy
/// these assertions. This test is ready to un-ignore once the real curated
/// artifact is authored by a human (T10 manual step).
///
/// # Human-authoring checklist (do this before removing `#[ignore]`)
///
/// ```
/// 1. cargo run -p dat0-app
/// 2. Open Chinook (drag chinook.sqlite or use the sample button).
/// 3. Build a "Revenue by genre" bar chart:
///      - GROUP BY genre, SUM(UnitPrice) per invoice_line
///      - save it via the chart-save button
/// 4. Apply a small projection pipeline to one grid (hide/rename a column).
/// 5. Open SQL Console → new tab → paste "Top customers" JOIN query → do NOT run.
///    Save the query to the library ("Save query…") so it round-trips.
/// 6. File → Export as .dat0 Package… (IN-APP ONLY — cold CLI flattens lineage)
/// 7. cp <exported>.dat0 crates/dat0-app/assets/demo.dat0
/// 8. Adjust the constants / names below if needed (chart name, SQL fragment).
/// 9. Remove `#[ignore]` and run:
///      cargo test -p dat0-app --test onboarding demo_dat0_preserves_chart_lineage
/// ```
///
/// # D-025 escalation note
///
/// If, after removing `#[ignore]`, the test fails because all recipe tables are
/// classified as `Base` (no `Derived` entries), the artifact was likely cold-CLI
/// exported, which **flattens** derived-table lineage (D-025). Fix: re-export
/// via the **in-app** File → Export path (which captures the live engine's
/// `table_origins` map). If the in-app path also flattens lineage, file a

...
fn t0_focus_oracle_and_take_tour(cx: &mut TestAppContext) {
    ...
    // dispatch path. We click a NEUTRAL spot in the enriched band's top row — 60px
    // left of the take-tour button (which sits top-RIGHT after a flex-grow tagline
    // that has no click handler of its own) — so only the shell's focus handler
    // fires and no hero action is triggered. The point is derived from the
    // button's painted bounds (no hard-coded pixel constant).
    let tt_bounds = cx
    ...
    assert!(
        ...
        "clicking into the workspace must focus the shell (precondition for Tab)"
    );

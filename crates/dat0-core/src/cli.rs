...
pub enum PackageCmd {
    ...
    /// bundle. A headless, window-free exit is the whole point.
    Version,
    /// `dat0 --help` / `dat0 -h`. Prints the package-subcommand usage and
    /// exits 0 without touching GPUI, for the same reason as [`Self::Version`].
    Help,
...
pub fn parse(args: &[String]) -> Option<PackageCmd> {
    let verb = args.get(1)?;
    // Global flags, matched BEFORE the `VERBS` gate: they are not subcommands,
    // so clap never sees them and `VERBS.contains` would send them to the GUI.
    if verb == "--version" || verb == "-V" {
        return Some(PackageCmd::Version);
    }
    if verb == "--help" || verb == "-h" {
        return Some(PackageCmd::Help);
    }
...
pub fn run(cmd: PackageCmd) -> i32 {
    ...
    if let PackageCmd::Version = cmd {
        ...
        // user reporting `--version` output names a build we can locate.
        let info = crate::about::build_info::BuildInfo::current();
        println!("dat0 {} ({})", info.version, info.git_sha);
        return 0;
    }
    if let PackageCmd::Help = cmd {
        // Rendered from `cli_command()` itself rather than a hand-kept string,
        // so a new verb can never ship undocumented.
        println!("{}", cli_command().render_long_help());
        return 0;
    }
...
pub async fn run_async(cmd: PackageCmd) -> i32 {
    match cmd {
        ...
        // Version/Help are handled synchronously in `run()` before the tokio
        // runtime is built; they can never reach this async dispatch path.
        PackageCmd::Version | PackageCmd::Help => {
            unreachable!("Version/Help are handled in run() before the runtime")
        }
...
mod tests {
    ...
    fn version_flag_parses() {
        ...
        assert_eq!(
            ...
            Some(PackageCmd::Version)
        );
        assert_eq!(parse(&argv(&["dat0", "-V"])), Some(PackageCmd::Version));
    }

    #[test]
    fn help_flag_parses() {
        assert_eq!(parse(&argv(&["dat0", "--help"])), Some(PackageCmd::Help));
        assert_eq!(parse(&argv(&["dat0", "-h"])), Some(PackageCmd::Help));
    }

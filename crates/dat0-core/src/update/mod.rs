...
pub fn fetch_latest(api_url: &str) -> Result<String> {
    // egress-seam: bodyless GET against the GitHub Releases API. Not one of
    // the four seams SH1 enumerated — it is a FIFTH outbound call the plan's
    // survey missed, which is precisely what `tests/egress_seams.rs` exists to
    // catch. Counted here rather than allow-listed.
    crate::telemetry::egress::record_request(
        "GET",
        api_url,
        crate::telemetry::egress::header_line_bytes("User-Agent", "dat0-update-check")
            + crate::telemetry::egress::header_line_bytes("Accept", "application/vnd.github+json"),
        0,
    );

...
fn get_bytes(url: &str) -> Result<Vec<u8>> {
    let mut buf = Vec::new();
    use std::io::Read;
    // egress-seam: a bodyless GET. The bytes that leave are the request line
    // plus the one header the updater sets — small, but not zero, and the
    // privacy claim is about whether anything left at all, not how much.

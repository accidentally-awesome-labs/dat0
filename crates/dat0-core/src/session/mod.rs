...
impl Session {
    ...
    pub async fn new(state_root: &Path, engine_budget_bytes: u64) -> Result<Self> {
        Self::new_with_id(state_root, engine_budget_bytes, Uuid::now_v7()).await
    }

    /// EN4: [`Session::new`] with the window id supplied by the caller.
    ///
    /// The async boot opens the window BEFORE the session exists, so the id has
    /// to be minted first: it keys the `WindowRegistry` entry, names the scratch
    /// directory, and discriminates the grid's TEMP VIEW. Handing the same value
    /// to both sides is what keeps a `Booting` window's registry entry pointing
    /// at the session it is about to receive.
...
mod tests {
    ...
    async fn ui_round_trips_through_persist_and_recover() {
        ...
        assert_eq!(
            layout.left_panel,
            Some(crate::session::dock_layout::LeftPanel::Catalog),

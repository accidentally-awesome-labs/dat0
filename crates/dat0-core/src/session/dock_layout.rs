...
//!
//! ## Why dat0 never calls `DockArea::load`
//!
//! MT1 deleted `panels::register_panels`, whose doc block used to be the only
//! place this was written down; a future drag-rearrange slice will look here,
//! not there.
//!
//! `DockAreaState::center` is a non-`Option` field and `load` unconditionally
//! rebuilds it through `to_item`, re-wrapping a `PanelInfo::Panel` as
//! `DockItem::tabs` — restoring the 30 px title bar, the nested scroll and the
//! `.cached()` child that B5 chose `DockItem::Panel` specifically to avoid.
//! There is no partial-load API, and B9's own T0 probe 2 confirmed the live
//! dump really does carry a centre named `"GridPanel"`, so this is a live
//! constraint rather than a theoretical one. dat0 persists a typed
//! [`DockLayout`] instead, which has no field a centre could occupy.
//!
//! The seven `PANEL_NAME` consts survive because they are the serialization
//! contract a rearrange slice would resolve panels through — independent of the
//! registry, and each frozen by its own `panel_name_is_frozen` test. That slice
//! has to solve the centre problem first, at which point re-registering
//! builders is its natural seam.
...
/// Which left-rail panel is showing.
///
/// The shell's own visibility bools remain the storage; this names the choice
/// they encode so every transition goes through one place and the
/// at-most-one-visible invariant is structural.
///
/// It lives here, beside the schema that persists it, rather than being
/// mirrored into a parallel session-side enum — panel identity has exactly one
/// definition.
...
pub struct DockLayout {
    ...
    /// Whether the catalog sidebar is showing (⌘B). Defaults **open**: a
    /// workbench with no visible catalog on first launch looks broken.
    ///
    /// This is the Dioxus shell's replacement for [`left_panel`](Self::left_panel):
    /// S1 folds the three-way mode switch into one always-present sidebar with
    /// three simultaneously-visible sections, so what used to be "which panel"
    /// is now "which sections are collapsed". `left_panel` and `left_size`
    /// remain only while the GPUI shell still reads them.
    #[serde(default = "yes")]
    pub sidebar_open: bool,
    /// User-resized sidebar width. `None` = the design's 238px.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub sidebar_size: Option<u32>,
    /// Which sidebar sections are collapsed, by [`section`](crate::catalog::nav)
    /// name. A set rather than three bools: sections are named data, and a new
    /// section must not mean a schema change.
    #[serde(default, skip_serializing_if = "std::collections::BTreeSet::is_empty")]
    pub sections_collapsed: std::collections::BTreeSet<String>,
}

/// `serde(default)` for a bool that defaults **on**.
fn yes() -> bool {
    true
}

/// Hand-written so the derive and `serde(default)` cannot disagree: a derived
/// `Default` would leave the sidebar closed while a file with no `sidebar_open`
/// key opened it.
impl Default for DockLayout {
    fn default() -> Self {
        Self {
            ...
            sidebar_open: true,
            sidebar_size: None,
            sections_collapsed: Default::default(),
        }
    }
}

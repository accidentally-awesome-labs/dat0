...
pub async fn run_connect(engine: Arc<DuckDBEngine>, token: String) -> ConnectionStatus {
    // egress-seam: MotherDuck. This is the one seam dat0 cannot meter.
    //
    // The ATTACH hands the token to DuckDB's MotherDuck extension, which then
    // opens and owns its own connection to the service: every subsequent query
    // dat0 routes at an `md:` database leaves this machine over a socket dat0
    // never sees. Recording a byte count for that traffic would be a fiction,
    // so the counter records only the credential handoff dat0 itself
    // originates and latches the "unmetered" flag — after this, the status
    // bar's egress figure is a FLOOR and says so with a `+`.
    let token_bytes = token.len() as u64;
    let opts = AttachOpts {
        token: Some(token),
        ..Default::default()
    };
    crate::telemetry::egress::record_sent(token_bytes);
    crate::telemetry::egress::note_unmetered_channel();

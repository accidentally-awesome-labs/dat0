...
pub async fn fetch_remote(
    ...
    // egress-seam: bodyless GET for the remote sample. `reqwest::get` sets no
    // headers of its own that dat0 chose, so the request line is the whole of
    // what dat0 originates; the multi-hundred-MB payload coming back is
    // ingress and is not counted here.
    crate::telemetry::egress::record_request("GET", url, 0, 0);

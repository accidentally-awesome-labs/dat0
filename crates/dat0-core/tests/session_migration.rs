...
use dat0_core::session::{SESSION_SCHEMA_VERSION, SessionState, migrate};
...
#[test]
fn v2_fixture_migrates_to_v7_preserving_content() {
    // v2 → v7 is an identity migration (T13): the version stamp bumps to the
...
async fn recover_eagerly_writes_back_current_version_on_first_open() {
    ...
    let _session = dat0_core::session::Session::recover(scratch_dir.clone(), BUDGET)
        .await
...
// ---------------------------------------------------------------------------
// T13 — v2 → current identity migration + unknown-`kind` forward-incompat guard
// ---------------------------------------------------------------------------
...
fn v2_session_migrates_to_v7_identity() {
    ...
    assert_eq!(
        derived,
        serde_json::to_value(dat0_core::session::dock_layout::DockLayout::default()).unwrap(),
...
#[test]
fn v3_with_projection_transform_loads_as_v7() {
    // A v3 file that ALSO carries a P4c projection transform must load (the
    // kinds are now known) and stamp v7 (current).
    let json = r#"{
      "schema_version": 3,
      "tabs": [{
        "table_name": "orders",
...
#[test]
fn v4_migration_to_v7_defaults_sql_tabs() {
    let v4_json = r#"{
      "schema_version": 4,
      "tabs": [
    ...
    assert!(state.sql_tabs.is_empty(), "v4->v7: sql_tabs defaults empty");
    assert_eq!(
        state.active_sql_tab, None,
        "v4->v7: active_sql_tab defaults None"
    );
}

#[test]
fn v5_round_trips_sql_tabs() {
    // A v5 file (with SQL tabs but no query stores) migrates to v7, stamping the
    // version while preserving its SQL tabs verbatim.
    let v5_json = r#"{
      "schema_version": 5,
      "tabs": [],
      "active_tab": null,
...
// ---------------------------------------------------------------------------
// T3 (P5b) — session v6 (additive query stores `query_history` + `saved_queries`)
// ---------------------------------------------------------------------------

#[test]
fn v5_file_migrates_to_v7_with_empty_query_stores() {
...
fn v6_roundtrips_query_stores() {
    let raw = r#"{"schema_version":6,"tabs":[],"active_tab":null,"sql_tabs":[],"active_sql_tab":null,
        ...
        "saved_queries":[]}"#;
    let state = migrate::load_str(raw).expect("v6 loads");
    assert_eq!(state.query_history.len(), 1);
    assert_eq!(state.query_history[0].sql, "select 1");
}

// ---------------------------------------------------------------------------
// T7 (P5c) — session v7 (additive `attachments` — MotherDuck + sqlite)
// ---------------------------------------------------------------------------

#[test]
fn v6_file_migrates_to_v7_with_empty_attachments() {
    // A v6 document (has query stores but no `attachments`) migrates to v7 via
...
fn v7_file_migrates_to_v8_with_default_ui() {
    ...
    assert_eq!(
        state.dock_layout,
        Some(dat0_core::session::dock_layout::DockLayout::default()),
...
fn v8_loads_dropping_dead_tree_ui() {
    ...
    assert_eq!(
        layout.left_panel,
        Some(dat0_core::session::dock_layout::LeftPanel::Catalog),
...
fn session_json_wire_format_is_snapshot_gated() {
    ...
    let state = SessionState {
        ...
        dock_layout: Some(dat0_core::session::dock_layout::DockLayout {
            left_panel: Some(dat0_core::session::dock_layout::LeftPanel::Catalog),
            left_size: Some(500),
            ...
            // Non-default on purpose: the snapshot has to prove the Dioxus
            // shell's sidebar state is on the wire and round-trips.
            sidebar_open: false,
            sidebar_size: Some(280),
            sections_collapsed: ["connections".to_string()].into_iter().collect(),
        }),
    };

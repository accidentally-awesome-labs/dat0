...
use dat0_core::session::{SESSION_SCHEMA_VERSION, SessionState, Tab, migrate};
use dat0_core::view::column_view::fold_columns;
use dat0_core::view::export_dialog::{ExportScope, build_export};
use dat0_core::view::{ViewChange, ViewModel};
...
/// Total rows returned by a view (via `execute_paged`'s `total_rows`).
///
/// `total_rows` is `Option<u64>` since EN1; `execute_paged` (as opposed to
/// `execute_page`) always populates it, so unwrapping here is asserting that
/// contract, not papering over an absence.
async fn view_row_count(engine: &DuckDBEngine, view: &str) -> u64 {
    engine
        ...
        .unwrap()
        .total_rows
        .expect("execute_paged always reports a total")
}
...
async fn projection_filter_export_remove_undo_restore() {
    ...
    assert_eq!(
        ...
        "reloaded schema_version must be the current schema (11)"
    );
    ...
    // On-disk session.json declares the current schema (v11).

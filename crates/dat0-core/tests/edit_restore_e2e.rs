...
async fn apply_change(engine: &DuckDBEngine, change: &dat0_core::view::ViewChange) {
...
async fn edit_delete_filter_round_trip_through_v3_session() {
    ...
    let paged_pre = engine
        ...
        .unwrap();
    assert_eq!(
        paged_pre.total_rows,
        Some(9),
        "10 rows − 1 deleted (id=0) = 9"
    );
    ...
    assert_eq!(
        ...
        "reloaded schema must be the current version"
    );

...
fn session_round_trips_sql_tabs_via_setter() {
    use dat0_core::session::SqlTabState;
    use uuid::Uuid;
    // Exercise the serialized shape directly:
    let state = dat0_core::session::SessionState {
        schema_version: dat0_core::session::SESSION_SCHEMA_VERSION,
        tabs: vec![],
        active_tab: None,
    ...
    let back: dat0_core::session::SessionState = serde_json::from_str(&json).unwrap();
